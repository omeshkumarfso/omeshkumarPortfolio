...
export default function About() {
  ...
  return (
    <section id="about" className="py-20 bg-linear-to-b from-slate-900 to-slate-800">
      <div className="container mx-auto px-4">
        ...
        >
          {stats.map((stat, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="text-center p-6 bg-linear-to-br from-slate-800/50 to-slate-700/50 rounded-2xl backdrop-blur-sm border border-emerald-500/20 hover:border-emerald-400/50 transition-all duration-300"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {highlights.map((highlight, index) => (
            ...
            >
              <div
                className={`absolute inset-0 bg-linear-to-r ${highlight.bgColor} rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
              ></div>
              <div className="relative bg-linear-to-br from-slate-800/80 to-slate-700/80 p-8 rounded-2xl backdrop-blur-sm border border-slate-600/50 hover:border-emerald-500/50 transition-all duration-300 transform hover:scale-105">
                <div className={`inline-flex p-3 rounded-xl bg-linear-to-r ${highlight.color} mb-4`}>
        ...
        >
          ...
          <div className="relative bg-linear-to-br from-slate-800/60 to-slate-700/60 p-8 md:p-12 rounded-3xl backdrop-blur-sm border border-emerald-500/20">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {["Team Leadership", "System Architecture", "AI Integration", "Performance Optimization"].map(
                (skill, index) => (
                  <span
                    key={index}
                    className="px-4 py-2 bg-linear-to-r from-emerald-500/20 to-teal-500/20 rounded-full text-emerald-300 text-sm font-medium border border-emerald-500/30"
                  >
                    {skill}
                  </span>
                ),
